...
export class CreateTaskComponent {
  ...
  // @Input() dataCT: any;
  @Input() timeEntryDetailData: any;
  @Input() isPayrollToBeFilledZeroHours: any = false;

  @Output() addedTask: EventEmitter<any> = new EventEmitter();
  @Output() allocationMessageChatBot: EventEmitter<any> = new EventEmitter();
  ...
  private allocationMessage: string;
    ...
    this.httpProvider.http.zentsCommonService({ url, data, addTask: true }).subscribe((res: any) => {
      ...
      if (res && res.data) {
        ...
        if (this.projectList && this.projectList.length > 0) {
          ...
          this.allocationMessageChatBot.emit({ allocationMessage: res.allocationMessage })
  ...
  addProjectTaskService(data) {
    ...
    this.httpProvider.http.zentsCommonService({ url, data, addTask: true }).subscribe((res: any) => {
      if (res && res.userMessage) {
        this.utility.updateLoader(false);
        this.addedTask.emit({ addedTask: true, CTforDate: this.currentDate })
